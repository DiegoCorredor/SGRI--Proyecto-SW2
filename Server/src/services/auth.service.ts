...
class AuthService {
  ...
  public async login(userData: CreateUserDto): Promise<{ cookie: string; findUser: User }> {
    ...
    const findUser: User = await this.users.findOne({ where: { mailUser: userData.mailUser }});
    if (!findUser) throw new HttpException(409, `This email ${userData.mailUser} was not found`);

    const isPasswordMatching: boolean = await compare(userData.passwordUser, findUser.passwordUser);
    if (!isPasswordMatching) throw new HttpException(409, "Password not matching");

    const tokenData = this.createToken(findUser);
    const cookie = this.createCookie(tokenData);

    findUser.passwordUser = '';
  ...
  public async logout(userData: User): Promise<User> {
    ...
    const findUser: User = await this.users.findOne({ where: { mailUser: userData.mailUser, passwordUser: userData.passwordUser } });
